...
class _StadiumDetailScreenState extends State<StadiumDetailScreen> {
  ...
  void _scrollToToday() {
    try {
      ...
      if (state is! BookingLoaded || state.stadium.fields == null) {
        return;
      }

      final selectedField = state.stadium.fields?.firstWhere(
      ...
      if (selectedField == null ||
          selectedField.bookings == null ||
          selectedField.bookings!.isEmpty) {
        return; // Exit if no bookings available.
      }

      if (selectedField?.bookings == null || selectedField!.bookings!.isEmpty) {
        return;
      }

      final today = DateFormat("yyyy-MM-dd").format(DateTime.now());
      final hasTodayBooking = selectedField.bookings!.any((booking) {
        return booking.timeSlot.startTime != null &&
            DateFormat("yyyy-MM-dd").format(booking.timeSlot.startTime!) ==
                today;
      });
      ...
      debugPrint('Error in _scrollToToday: $e');
    }
  }
